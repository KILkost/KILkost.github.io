...
document.getElementById('loadPlaylist').addEventListener('click', () => {
  ...
  if (url) {
    ...
    if (list) {
      ytIframe.src = `https://www.youtube.com/embed?listType=playlist&list=${encodeURIComponent(list)}&rel=0`;
      return;
    }
    if (video) {
      ytIframe.src = `https://www.youtube.com/embed/${encodeURIComponent(video)}?rel=0`;
      return;
    }
  }
    ...
    await new Promise((r, rej) => {
      ...
      reader.onload = () => {
        store.add({
          name: f.name,
          type: f.type,
          blob: new Blob([reader.result], { type: f.type }),
          date: Date.now()
        });
        r();
      };
...
async function refreshList() {
  ...
  container.innerHTML = '';
  if (list.length === 0) {
    container.innerHTML = '<div style="font-size:12px;color:#ccc">Нет треков</div>';
    return;
  }
  list.forEach(item => {
